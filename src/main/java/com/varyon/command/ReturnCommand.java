...
public class ReturnCommand extends AbstractPlayerCommand {
    ...
    public ReturnCommand() {
        ...
        this.addUsageVariant(new ForceVariant());
    }

    @Override
    protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store,
                           @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
        executeReturn(context, store, ref, playerRef, world, false);
    }

    private class ForceVariant extends CommandBase {
        private final RequiredArg<String> confirmArg;

        ForceVariant() {
            super("Force le /return pendant le cooldown (coût doublé)");
            this.confirmArg = this.withRequiredArg("force", "Tapez 'force' pour confirmer", ArgTypes.STRING);
        }
        ...
        protected void executeSync(@Nonnull CommandContext context) {
            ...
            if (!context.get(confirmArg).equalsIgnoreCase("force")) {
                context.sendMessage(Message.raw("Usage : /return force").color(Color.RED));
                return;
            }
            ...
            world.execute(() -> {
                ...
                executeReturn(context, store, ref, playerRef, world, true);
            });
        }
    }

    void executeReturn(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store,
                       @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world,
                       boolean isForce) {
        ...
        boolean onCooldown = deathManager.isOnCooldown(playerRef.getUuid());
        ...
        final BigDecimal costBD = BigDecimal.valueOf(totalCost);

        if (onCooldown && !isForce) {
            long remaining = deathManager.getCooldownRemainingSeconds(playerRef.getUuid());
            String warning = msg.cooldownWarning
                .replace("{remaining}", String.valueOf(remaining))
                .replace("{cost}", String.valueOf(totalCost))
                .replace("{multiplier}", String.valueOf(multiplier));
            context.sendMessage(Message.raw(warning).color(Color.YELLOW));
            return;
        }
        ...
        // Mise à jour d'état synchrone : empêche tout spam avant que le lambda s'exécute
        ...
        LOGGER.at(Level.INFO).log("Return initiated for " + playerRef.getUuid() +
            " zone=" + (zone != null ? zone.getName() : "none") +
            " cost=" + totalCost + " multiplier=x" + multiplier);

        final boolean wasOnCooldown = onCooldown;
        ...
        world.execute(() -> {
            try {
                ...
                String costSuffix = rtpvConfig.isEconomyEnabled()
                    ? " (-" + totalCost + " coins)" : "";
                context.sendMessage(Message.raw(successMsg + costSuffix).color(Color.GREEN));
